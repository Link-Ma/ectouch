...
class IndexController extends CommonController
{
    ...
    {
        load('com/lib_article');
        $list = get_article_count(11);
        dump($list);
        exit;
        /* 缓存编号 */
    ...
    private function index_get_group_buy()
    {
        ...
        if ($limit > 0) {
            $sql = 'SELECT gb.act_id AS group_buy_id, gb.goods_id, gb.ext_info, gb.goods_name, g.goods_thumb, g.goods_img ' . 'FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS gb, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . "WHERE gb.act_type = '" . GAT_GROUP_BUY . "' " . "AND g.goods_id = gb.goods_id " . "AND gb.start_time <= '" . $time . "' " . "AND gb.end_time >= '" . $time . "' " . "AND g.is_delete = 0 " . "ORDER BY gb.act_id DESC " . "LIMIT $limit";
    ...
    private function index_get_auction()
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            $arr['url'] = build_uri('auction', array(
                'auid' => $arr['act_id']
            ));
